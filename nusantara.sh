...
EOF
    ...
    git clone https://github.com/ganendra4u/android_device_sony_apollo -b nusantara-a12 device/sony/xz2c
    git clone https://github.com/ganendra4u/android_device_sony_tama-common -b a12 device/sony/tama-common
    git clone https://github.com/LineageOS/android_hardware_sony_SonyOpenTelephony -b lineage-19.0 hardware/sony/SonyOpenTelephony
    git clone https://github.com/TheMuppets/proprietary_vendor_sony_xz2c -b lineage-20 vendor/sony/apollo
    git clone https://github.com/TheMuppets/proprietary_vendor_sony_tama-common -b lineage-20 vendor/sony/tama-common

ulimit -n 16000

COMMON_MK=device/sony/tama-common/common.mk

if ! grep -A3 "PRODUCT_SOONG_NAMESPACES" $COMMON_MK | grep -q "vendor/sony/tama-common"; then
  sed -i '/^PRODUCT_SOONG_NAMESPACES += \\$/a\    vendor/sony/tama-common \\' $COMMON_MK
fi

if grep -A1 "^PRODUCT_BOOT_JARS += \\\\$" $COMMON_MK | grep -q "^[[:space:]]*WfdCommon[[:space:]]*$"; then
  sed -i '/^PRODUCT_BOOT_JARS += \\$/{N;/WfdCommon/d}' $COMMON_MK
fi
