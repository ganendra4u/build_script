...
start_build_process() {
    ...
    repo init --depth=1 --no-repo-verify -u https://github.com/AfterLifePrjkt13/android_manifest.git -b 13x -g default,-mips,-darwin,-notdefault --depth=1
...
sed -i "s/tama_${DEVICE_CODE}_dcm_defconfig/tama_${DEVICE_CODE}_defconfig/" device/sony/"$DEVICE_CODE"/BoardConfig.mk

echo "Renaming device makefile to xxx naming convention..."
cd device/sony/"$DEVICE_CODE"
if [ -f lineage_"$DEVICE_CODE".mk ]; then
    mv lineage_"$DEVICE_CODE".mk arrow_"$DEVICE_CODE".mk
fi
sed -i "s/PRODUCT_NAME := lineage_${DEVICE_CODE}/PRODUCT_NAME := afterlife_${DEVICE_CODE}/" afterlife_"$DEVICE_CODE".mk
sed -i 's#vendor/lineage/config/common_full_phone.mk#vendor/afterlife/config/common.mk#' afterlife_"$DEVICE_CODE".mk
sed -i "s/lineage_${DEVICE_CODE}/afterlife_${DEVICE_CODE}/g" AndroidProducts.mk

if ! grep -q "DEVICE_MAINTAINER" afterlife_"$DEVICE_CODE".mk; then
    cat >> afterlife_"$DEVICE_CODE".mk << 'EOF'

AFTERLIFE_MAINTAINER := ganendra1945
EOF
fi
cd -
