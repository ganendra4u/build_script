...
start_build_process() {
    ...
    # Manifest AxionOS, branch lineage-23.2
    repo init -u https://github.com/AxionAOSP/android.git -b lineage-23.2 --git-lfs --no-clone-bundle
    ...
    DEVICE_MK="device/sony/$DEVICE_CODE/lineage_$DEVICE_CODE.mk"
    if [ -f "$DEVICE_MK" ]; then
        # Avoid duplicate injection if script is re-run
        if ! grep -q "AXION_MAINTAINER" "$DEVICE_MK"; then
            cat >> "$DEVICE_MK" <<EOF

# --- AxionOS device properties (auto-injected by build script) ---
...
EOF
            echo "AxionOS properties appended to $DEVICE_MK"
        else
            echo "AxionOS properties already present in $DEVICE_MK, skipping."
        fi
    else
        echo "WARNING: $DEVICE_MK not found. Check the actual device makefile name and add AXION_MAINTAINER / AXION_CAMERA_*_INFO manually." >&2
    fi
